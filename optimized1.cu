...
void Policy_Calculation_Kernel(float *returns, int *policy, float *stateValue, float *poisson) {
	...
	if (threadIdx.x == 0 && threadIdx.y == 0) {
		returns[(blockIdx.x*POISSON_UP_BOUND+threadIdx.x)*(MAX_CARS + 1)*POISSON_UP_BOUND + blockIdx.y*POISSON_UP_BOUND+threadIdx.y] = returns[(blockIdx.x*POISSON_UP_BOUND+threadIdx.x)*(MAX_CARS + 1)*POISSON_UP_BOUND + blockIdx.y*POISSON_UP_BOUND+threadIdx.y] - MOVE_CAR_COST * abs(policy[blockIdx.x*(MAX_CARS + 1) + blockIdx.y]);
	...
	for (returnedCarsFirstLoc = 0; returnedCarsFirstLoc < POISSON_UP_BOUND; returnedCarsFirstLoc++) {
		for (returnedCarsSecondLoc = 0; returnedCarsSecondLoc < POISSON_UP_BOUND; returnedCarsSecondLoc++) {
			...
			returns[(blockIdx.x*POISSON_UP_BOUND+threadIdx.x)*(MAX_CARS + 1)*POISSON_UP_BOUND + blockIdx.y*POISSON_UP_BOUND+threadIdx.y] = returns[(blockIdx.x*POISSON_UP_BOUND+threadIdx.x)*(MAX_CARS + 1)*POISSON_UP_BOUND + blockIdx.y*POISSON_UP_BOUND+threadIdx.y] + prob * (reward + DISCOUNT * stateValue[numOfCarsFirstLoc*(MAX_CARS + 1) + numOfCarsSecondLoc]);
		}
	}
...
void Policy_Calculation_Kernel2(float *returns, int action, float *stateValue, float *poisson) {
	...
	if (action <= a && (-action) <= b) {
		...
		//cost for moving cars
		if (threadIdx.x == 0 && threadIdx.y == 0) {
			returns[(blockIdx.x*POISSON_UP_BOUND+threadIdx.x)*(MAX_CARS + 1)*POISSON_UP_BOUND + blockIdx.y*POISSON_UP_BOUND+threadIdx.y] = returns[(blockIdx.x*POISSON_UP_BOUND+threadIdx.x)*(MAX_CARS + 1)*POISSON_UP_BOUND + blockIdx.y*POISSON_UP_BOUND+threadIdx.y] - MOVE_CAR_COST * abs(action);
		}
